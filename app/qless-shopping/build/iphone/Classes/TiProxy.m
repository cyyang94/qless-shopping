...
//Rare exceptions to indicate a bug in the _qless_shopping code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_qless_shopping = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_qless_shopping]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.qless_shopping.module"
